...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <small className="version">V6.7.2 UI Restore & Real Controls Fix · PRO DEV</small>
    </aside>
    <main className="main">
      ...
      {tab==='settings' && <SettingsPanel licenseState={licenseState} t={t} settings={settings} updateSettings={updateSettings} perf={perf} logsPath={logsPath} runAudit={runAudit} openLogs={openLogs} debugLoading={debugLoading} applyQuality={applyQuality} updateInfo={updateInfo} updateLoading={updateLoading} checkUpdatesNow={checkUpdatesNow} licenseKey={licenseKey} setLicenseKey={setLicenseKey} owner={licenseOwner} setOwner={setLicenseOwner} licenseMsg={licenseMsg} activateLicense={activateCurrentLicense} continueFree={continueFree}/>} 
...
function Dashboard({t, matrixInputs, outputs, apps, perf, timestamp, routes}: any) {
  const route = routes?.[0] || DEFAULT_ROUTES[0];
  const routeOutputs = route.outputs && route.outputs.length === 5 ? route.outputs : defaultRouteOutputs(outputs || []);
  const outputName = (idx:number) => {
    const id = routeOutputs[idx];
    const found = (outputs || []).find((d:AudioDevice)=>d.id === id);
    return found?.name || (t.noAudioDevice || 'Nenhum dispositivo');
  };
  const virtualSources = VIRTUAL_ROUTE_KEYS.map(k => ({
    key:k,
    sources:[
      ...(matrixInputs||[]).filter((x:any)=>normalizeRoute(x.routing)[k]).map((x:any)=>({name:x.name, peak:n(x.peak || sourcePeak(x.id)), muted:x.muted})),
      ...(apps||[]).filter((x:any)=>normalizeRoute(x.routing)[k]).map((x:any)=>({name:x.app || x.process, peak:n(x.peak), muted:isEffectivelyMuted(x)})),
    ]
  }));
  const outputSources = HARDWARE_ROUTE_KEYS.map((k, idx) => ({ key:k, device: outputName(idx), sources:(apps||[]).filter((x:any)=>normalizeRoute(x.routing)[k]) }));
  const alerts = [
    ...(routeOutputs || []).map((id:string,idx:number)=> id && !(outputs||[]).some((d:AudioDevice)=>d.id===id) ? `A${idx+1}: ${t.unavailable || 'Indisponível'}` : '').filter(Boolean),
    ...(matrixInputs || []).map((x:any)=> x.deviceId && !(x.pro) && !(x.deviceId && true) ? `${x.name}: ${t.unavailable || 'Indisponível'}` : '').filter(Boolean)
  ];
  return <section>
    ...
    <div className="dashboard-bus-layout">
      <div className="panel"><h3>Hardware IN</h3><p className="hint">{t.hardwareInHelp || 'Fontes enviadas para saídas virtuais.'}</p><div className="bus-grid five">{virtualSources.map(ch=><div className="bus-monitor" key={ch.key}>{ch.sources.length ? ch.sources.slice(0,5).map((s:any,idx:number)=><span className={s.muted?'off-app':''} title={s.name} key={`${ch.key}-${idx}`}>{s.name}</span>) : <small>{t.empty || 'Vazio'}</small>}<ThemeVuMeter value={ch.sources.reduce((a:any,b:any)=>Math.max(a,n(b.peak)),0)} /></div>)}</div></div>
      <div className="panel"><h3>Hardware OUT</h3><p className="hint">{t.hardwareOutHelp || 'Dispositivos físicos e apps conectados em cada saída.'}</p><div className="bus-grid five">{outputSources.map((ch:any,idx:number)=><div className="bus-monitor" key={ch.key}><b title={ch.device}>{`A${idx+1}`}</b><small className="device-name" title={ch.device}>{ch.device}</small>{ch.sources.length ? ch.sources.slice(0,4).map((s:any)=><span key={sessionKey(s)}>{s.app || s.process}</span>) : <small>{t.empty || 'Vazio'}</small>}<ThemeVuMeter value={ch.sources.reduce((a:any,b:any)=>Math.max(a,n(b.peak)),0)} /></div>)}</div></div>
    </div>
    <div className="panel alerts-panel"><h3>{t.alerts || 'Alertas'}</h3>{alerts.length ? alerts.map((a:string)=><span className="alert-chip" key={a}>{a}</span>) : <span className="ok">{t.ok || 'OK'}</span>}</div>
...
function Devices({t, inputs, matrixInputs, updateMatrixInput, outputs, routes, setRoutes, selected, reconnecting, setReconnecting, refreshMain}: any) {
  ...
  const setBus = (idx: number, value: string) => {
    ...
    if (value) {
      for (let i=0;i<nextOutputs.length;i++) if (i !== idx && nextOutputs[i] === value) nextOutputs[i] = '';
    }
    nextOutputs[idx] = value;
    nextRoutes[selected || 0] = { ...current, outputs: nextOutputs };
    setRoutes(nextRoutes);
  };
  const outputOptionsFor = (idx:number) => {
    const used = new Set(routeOutputs.filter((id:string, i:number)=>i !== idx && id));
    return [...outputs.filter((d:AudioDevice)=>!used.has(d.id || '')).map((d: AudioDevice) => [d.id || '', d.name || t.noAudioDevice]), ['', t.noAudioDevice || 'Nenhum dispositivo']];
  };
  ...
  return <section><h2>{t.devicesTitle || 'Dispositivos'}</h2><p className="sub">{t.devicesCleanSub || 'Configure as entradas e as saídas físicas do Miller.'}</p>
    ...
    <div className="panel"><h3>{t.outputs || 'Saídas físicas'}</h3><p className="hint">{t.outputUniqueHelp || 'O mesmo dispositivo físico só pode ficar em uma saída. Se escolher em outra, ele é removido da anterior automaticamente.'}</p><div className="device-card-grid outputs-five">{[0,1,2,3,4].map(i=>{ const unavailableBus = !!routeOutputs[i] && !outputIds.has(routeOutputs[i]); const label = routeOutputLabel(routeOutputs[i], i, outputs, t); return <div className={unavailableBus ? 'output-slot unavailable blink-box' : 'output-slot'} key={i}><h3>A{i+1}</h3><label>{t.device || 'Dispositivo'}<select value={outputIds.has(routeOutputs[i]) ? routeOutputs[i] : ''} onChange={e=>setBus(i,e.target.value)}>{outputOptionsFor(i).map(([id, label]: any, n: number)=><option key={`${id}-${n}`} value={id}>{label}</option>)}</select></label><label>{t.volume || 'Volume'}<input className="volume-slider wide" type="range" min="0" max="100" defaultValue="100"/><small>100%</small></label><DbSlider label="Gain dB" value={0} onChange={()=>{}} help="Ganho da saída física."/><small>{t.vuMeter || 'VU Meter'}</small><ThemeVuMeter value={0}/><small className="device-name" title={label}>{label}</small>{unavailableBus && <b className="off blink">{t.unavailable || 'Indisponível'}</b>}<button onClick={reconnect} disabled={reconnecting}>{reconnecting ? (t.reconnecting || 'Reconectando...') : (t.reconnect || 'Reconectar')}</button></div>})}</div></div>
  </section>;
}
  ...
  return <section>
    ...
    <div className="panel app-lines-panel">
      {rows.length ? <table className="app-lines"><thead><tr><th>{t.apps || 'Aplicativo'}</th><th>{t.volume || 'Volume'}</th><th>Gain dB</th><th>{t.vuMeter || 'VU Meter'}</th><th>A1-A5 / B1-B5</th><th>{t.mute || 'Mute'}</th><th>{t.solo || 'Solo'}</th><th>FX</th></tr></thead><tbody>{rows.map((a:any,i:number)=>{ const routing = normalizeRoute(a.routing); const key = `${a.key || a.process}-${a.pid || i}`; const open = expanded === key; return <>
        <tr key={key} className={a.active ? '' : 'inactive-app'}><td><b title={a.app || a.process || '—'}>{a.app || a.process || '—'}</b>{!a.active && <small className="off"> · {t.silent || 'Sem áudio'}</small>}</td><td><input className="volume-slider" type="range" min="0" max="100" value={n(isEffectivelyMuted(a) ? 0 : a.volume)} onChange={e=>updateRow(a,{volume:Number(e.target.value), muted:false})}/><small>{n(isEffectivelyMuted(a) ? 0 : a.volume)}%</small></td><td><DbSlider label="" value={n(a.gainDb)} onChange={v=>updateRow(a,{gainDb:v})}/></td><td><ThemeVuMeter value={n(a.peak)} /><small>{n(a.peak)}%</small></td><td><div className="route-buttons compact-routes">{ROUTE_KEYS.map((k)=><button key={k} className={routing[k]?'circle on':'circle'} onClick={()=>updateRow(a,{routing:patchRoute(routing,k,!routing[k]), muted:false})}>{busLabel(k)}</button>)}</div></td><td><button className={isEffectivelyMuted(a)?'danger active':''} onClick={()=>updateRow(a,{muted:!a.muted})}>{isEffectivelyMuted(a) ? (t.unmute || 'Unmute') : (t.mute || 'Mute')}</button></td><td><button className={a.solo?'active':''} onClick={()=>updateRow(a,{solo:!a.solo})}>{t.solo || 'Solo'}</button></td><td><button onClick={()=>setExpanded(open?'':key)}>FX</button></td></tr>
...
function SettingsPanel({licenseState, t, settings, updateSettings, perf, logsPath, runAudit, openLogs, debugLoading, applyQuality, updateInfo, updateLoading, checkUpdatesNow, licenseKey, setLicenseKey, owner, setOwner, licenseMsg, activateLicense, continueFree}: any) {
  const limits = planLimits(licenseState);
  const allThemeOptions = [['miller-blue','Miller Blue'],['miller-red','Miller Red'],['miller-purple','Miller Purple'],['miller-green','Miller Green'],['miller-gold','Miller Gold'],['miller-orange','Miller Orange'],['miller-ice','Miller Ice'],['miller-carbon','Miller Carbon'],['miller-cyberpunk','Miller Cyberpunk'],['miller-midnight','Miller Midnight'],['miller-matrix','Miller Matrix'],['carbon-black','Carbon Black'],['cyber-neon','Cyber Neon'],['studio-pro','Studio Pro']];
  const themeOptions = limits.themes ? allThemeOptions.filter((x:any)=>(limits.themes as string[]).includes(x[0])) : allThemeOptions;
  return <section><h2>{t.settingsTitle}</h2><p className="sub">{t.settingsSub}</p>
    <div className="settings-grid restored-settings">
      <div className="panel"><h3>{t.general}</h3><Select label={t.language} value={settings.language} options={[['pt-BR','🇧🇷 Português (Brasil)'],['en-US','🇺🇸 English'],['es-ES','🇪🇸 Español']]} onChange={(v)=>updateSettings({language:v})}/><h3>{t.startup}</h3><Check label={t.startWithWindows} checked={settings.startWithWindows} onChange={(v)=>updateSettings({startWithWindows:v})}/><Check label={t.startMinimized} checked={settings.startMinimized} onChange={(v)=>updateSettings({startMinimized:v})}/><Check label={t.closeToTray} checked={settings.closeToTray} onChange={(v)=>updateSettings({closeToTray:v})}/></div>
      <div className="panel"><h3>{t.appearance}</h3><Select label={t.theme} value={settings.theme} options={themeOptions} onChange={(v)=>updateSettings({theme:v})}/><Select label={t.scale} value={settings.scale} options={[90,100,110,125]} onChange={(v)=>updateSettings({scale:Number(v)})}/><Check label={t.animations} checked={settings.animations} onChange={(v)=>updateSettings({animations:v})}/><Check label={t.transparency} checked={settings.transparency} onChange={(v)=>updateSettings({transparency:v})}/></div>
      <div className="panel"><h3>{t.licenseTitle || 'Licença'}</h3>{row(t.status, licenseState?.activated ? (licenseState?.licenseType || 'PRO') : (t.freePlan || 'Free'), licenseState?.activated?'ok':'warn')}{row(t.pcId || 'PC ID', licenseState?.machineId || machineId())}{row(t.remaining || 'Restante', licenseState?.activated ? '∞' : formatTimeMMSS(licenseState?.freeSecondsLeft || 0))}<label>{t.name || 'Nome'}<input value={owner || ''} onChange={(e:any)=>setOwner?.(e.target.value)} placeholder="Cliente"/></label><label>{t.licenseKey || 'Chave'}<input value={licenseKey || ''} onChange={(e:any)=>setLicenseKey?.(e.target.value)} placeholder="MASX-PRO-XXXX"/></label><div className="btn-row"><button onClick={activateLicense}>{t.activate || 'Ativar'}</button><button onClick={continueFree}>{t.continueFree || 'Continuar Free'}</button></div>{licenseMsg && <p className="hint">{translateMessage(licenseMsg, t)}</p>}</div>
      <div className="panel"><h3>{t.updates}</h3><Check label={t.checkUpdates} checked={settings.checkUpdates} onChange={(v)=>updateSettings({checkUpdates:v})}/><Select label={t.channel} value={settings.updateChannel} options={['stable','beta','dev']} onChange={(v)=>updateSettings({updateChannel:v})}/><button onClick={checkUpdatesNow} disabled={updateLoading}>{updateLoading?t.running:(t.checkNow || 'Verificar agora')}</button>{updateInfo && <div className="update-box">{row(t.currentVersion, updateInfo.currentVersion)}{row(t.latestVersion, updateInfo.latestVersion, updateInfo.updateAvailable?'warn':'ok')}<p className="hint">{translateMessage(updateInfo.message, t)}</p>{(updateInfo.notes||[]).map((note:string,i:number)=><small className="field-help" key={i}>{translateMessage(note, t)}</small>)}</div>}</div>
      <div className="panel"><h3>{t.audio}</h3><div className="quality-grid"><QualityCard active={settings.qualityMode==='lowLatency'} title={t.lowLatency} help={t.lowLatencyHelp} onClick={()=>applyQuality('lowLatency')}/><QualityCard active={settings.qualityMode==='balanced'} title={t.balanced} help={t.balancedHelp} onClick={()=>applyQuality('balanced')}/><QualityCard active={settings.qualityMode==='quality'} title={t.maxQuality} help={t.maxQualityHelp} onClick={()=>applyQuality('quality')}/></div><button onClick={()=>updateSettings({showAdvancedAudio:!settings.showAdvancedAudio})}>{settings.showAdvancedAudio?t.hideAdvanced:t.showAdvanced}</button>{settings.showAdvancedAudio && <div><Select label={t.engine} value={settings.engine} options={[['auto',t.automatic],['wasapi_shared','WASAPI Shared'],['wasapi_exclusive','WASAPI Exclusive'],['asio',t.futureAsio]]} onChange={(v)=>updateSettings({engine:v})}/><SmallHelp text={t.bufferHelp}/><Select label={t.buffer} value={settings.buffer} options={[64,128,256,512]} onChange={(v)=>updateSettings({buffer:Number(v)})}/><SmallHelp text={t.sampleRateHelp}/><Select label={t.sampleRate} value={settings.sampleRate} options={[44100,48000,96000]} onChange={(v)=>updateSettings({sampleRate:Number(v)})}/><SmallHelp text={t.aiHelp}/><Select label={t.aiEngine} value={settings.micEngine} options={[['off',t.disabled],['rnnoise','RNNoise'],['webrtc','WebRTC'],['deepfilternet','DeepFilterNet']]} onChange={(v)=>updateSettings({micEngine:v})}/></div>}</div>
      <div className="panel"><h3>{t.supportTitle || 'Suporte'}</h3><p className="hint">{t.supportSub || 'Diagnóstico, logs e relatório técnico para suporte.'}</p>{row(t.latency, latencyText(perf,t), latencyStatus(n(perf?.roundTripLatencyMs ?? perf?.latencyMs)))}{row(t.inputLatency || 'Input latency', `${n(perf?.inputLatencyMs)} ms`, latencyStatus(n(perf?.inputLatencyMs)))}{row(t.outputLatency || 'Output latency', `${n(perf?.outputLatencyMs)} ms`, latencyStatus(n(perf?.outputLatencyMs)))}{row(t.buffer, `${settings.buffer} ${t.samples}`)}{row(t.sampleRate, `${settings.sampleRate} Hz`)}{row(t.logsPath, logsPath || '—')}<div className="btn-row"><button onClick={openLogs}>{t.openLogs}</button><button onClick={runAudit} disabled={debugLoading}>{debugLoading?t.running:t.runAudit}</button></div></div>
    </div>
  </section>;
}
